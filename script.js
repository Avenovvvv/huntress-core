...
/* =========================
   TERMS POPUP
========================= */
function acceptTerms(){ window.location.href = "gateway-loading.html"; }
function denyTerms(){ window.location.href = "denied.html"; }
...
if(visitorElement){
    ...
    const hunterId = localStorage.getItem("hunterId") || "000";
    
    visitorElement.innerHTML = `
        <div style="font-size:15px; line-height:2.2;">
        YOU ARE VISITOR #${visitorCount}
        <br><br>
        CURRENT NODE: <span style="color:#ff335f;">HUNTER_${hunterId}</span>
        <br><br>
...
cards.forEach((card) => {
    card.addEventListener("mousemove", (e) => {
        ...
        card.style.background = `radial-gradient(circle at ${x}px ${y}px, rgba(255,0,0,0.12), rgba(10,10,10,0.96))`;
    });
...
========================= */
const revealElements = document.querySelectorAll(".reveal, .info-section");
...
/* =========================
   AUDIO: DOSSIER & COMMS
========================= */
const soundLinks = document.querySelectorAll(".blog-card, .comms-links a");
...
/* =========================================
   H.U.N.T.R.E.S.S AI LINK (GEMINI PROXY)
========================================= */
        ...
        if (e.key === "Enter") {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    terminalOutput.innerHTML += `<p class="error-log">> ERROR: ${data.error || 'UPLINK FAILED'}</p>`;
                }
            } catch (err) {
                const loader = document.getElementById("loading");
                if(loader) loader.remove();
